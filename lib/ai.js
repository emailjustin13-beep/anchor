...
export const DRAFT_SCAN_SCHEMA = {
  ...
  properties: {
    existing_issue_decisions: {
      ...
      items: {
        ...
        properties: {
          issue_id: string,
          status: { type: 'string', enum: ['still_open', 'resolved'] },
          decision_explanation: string,
          ...draftFindingProperties,
        },
        required: ['issue_id', 'status', 'decision_explanation', ...draftFindingRequired],
      },
    },
    new_findings: {
      type: 'array',
  ...
  required: ['existing_issue_decisions', 'new_findings', 'overall'],
}
...
- Return exactly one existing_issue_decisions entry for every ledger issue. Copy its exact issue_id and choose exactly one status: still_open or resolved.
...
- Use resolved when the conflict was removed, the current draft supplies a reasonable bridge, or the item is now only an unanswered question, intentional mystery, or general craft observation. A resolved decision cannot also appear in new_findings.
