  ...
  properties: {
    findings: {
      ...
      items: {
        ...
        required: ['category', 'priority', 'title', 'summary', 'question', 'previous_issue_id', 'plausible_inference', 'inference_explanation', 'possibilities', 'characters', 'evidence'],
      },
    },
...
- Every returned finding must set plausible_inference to false and briefly state in inference_explanation why the existing clues cannot reasonably bridge the conflict. Anchor defensively suppresses any returned item marked plausible_inference true.
...
- For a finding that is the same underlying issue as an existing ledger entry, put that exact ID in previous_issue_id.
- When an issue's evidence is unchanged, keep its underlying meaning stable so Anchor can preserve the writer-facing wording.
- When evidence materially changed but the issue remains, rewrite the title, summary, question, possibilities, and evidence so every claim accurately describes the current draft.
