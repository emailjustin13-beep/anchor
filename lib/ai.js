  ...
  properties: {
    findings: {
      ...
      items: {
        ...
        required: ['category', 'priority', 'title', 'summary', 'question', 'characters', 'evidence'],
      },
    },
    overall: string,
  },
...
export function buildDraftScanPrompt({ scriptText, characters, relationships, relationshipEvents = [], characterStateEvents = [] }) {
...
Never rewrite, recommend adding a scene, beat, explanation, or internal reaction, and never tell the writer how to fix the work. A surprising choice can be intentional development. Phrase every observation as a neutral question that helps the writer confirm intent. Report a contradiction only when the draft contains facts genuinely in tension. Use one or two short exact quotes as evidence and label their locations. Do not repeat the same underlying issue in multiple categories. Prefer specific continuity conflicts over general craft commentary. If the draft supports a reasonable explanation, acknowledge the ambiguity rather than declaring an error. If no meaningful issue exists, return an empty findings array.`,
