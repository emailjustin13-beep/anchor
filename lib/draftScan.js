...
export function buildDraftScanPrompt({
  ...
  const requiredIssueIds = previousIssues.map(issue => issue.id)
...
Do not assume two similar objects are the same object. Articles and ownership matter: "a pistol" does not automatically mean an earlier locked pistol, and a later generic key does not automatically mean the sole established key. Require text that identifies the object as the same, only, or otherwise exclusive object before treating its location or possession as incompatible. A recording can be played from a different location than where it was captured; a movable recorder's later location does not establish the recording location.

Treat an explicit on-page cause as a supported change. A shown death, execution, departure, breakup, discovery, betrayal, or stated decision can bridge a life-state, relationship, knowledge, or character shift even when the Story Bible snapshot still reflects the earlier state. An extreme event plus the character explicitly renouncing a former absolute boundary can earn a reversal. Do not flag the stale snapshot alone. If a later objective reveal establishes that an earlier report, identification, assumption, or apparent fact was forged, staged, false, or misattributed, use the reveal as current canon and resolve the old conflict.
...
- During an incremental scan, look for new issues only when they involve a changed scene. Use unchanged Story Memory only as comparison context.`,
    prompt:`SCAN MODE: ${incremental ? 'INCREMENTAL — analyze changed scenes against persistent memory' : 'INITIAL — establish the issue ledger'}\n\nCONFIRMED CHARACTERS:\n${charSummaries || 'None confirmed.'}\n\nCONFIRMED RELATIONSHIPS:\n${relSummaries || 'None confirmed.'}\n\nCONFIRMED RELATIONSHIP HISTORY:\n${relationshipHistory || 'No history confirmed.'}\n\nCONFIRMED CHARACTER-STATE HISTORY:\n${stateHistory || 'No state history confirmed.'}\n\nEXISTING ISSUE LEDGER — HISTORICAL, NOT CANONICAL:\n${issueLedger || 'No existing issues.'}\n\nREQUIRED LEDGER DECISIONS: ${requiredIssueIds.length}\nREQUIRED ISSUE IDS: ${requiredIssueIds.join(', ') || 'none'}\nYour existing_issue_decisions array must contain exactly ${requiredIssueIds.length} entries and copy each required issue ID exactly once. Do not omit a decision because an issue resolved, stayed unchanged, or was reworded. Do not place any required issue in new_findings.\n\nWRITER-REVIEWED AS NOT A PROBLEM:\n${reviewedFindings || 'None.'}\n\n${scanMaterial}\n\nFirst decide every existing ledger issue exactly once. Verify the decision count and exact IDs before analyzing new candidates. Then test every genuinely new candidate against the contradiction, mystery, character-evidence, and reasonable-audience gates. Return no more than five new_findings in stable priority order.`,
  }
}
