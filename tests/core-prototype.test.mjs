...
test('draft scans use a fast bounded request path with visible elapsed time', async () => {
  ...
  assert.match(route, /draft_scan:50000/)
  assert.match(route, /ANTHROPIC_DRAFT_SCAN_MODEL/)
  assert.match(route, /claude-opus-5/)
  assert.match(route, /thinking = \{ type:'adaptive', display:'omitted' \}/)
  assert.match(route, /output_config = \{ effort:'medium' \}/)
  assert.match(route, /AI_TIMEOUT/)
  assert.match(client, /controller\.abort/)
  assert.match(editor, /profile:'draft_scan'/)
  assert.match(editor, /maxTokens:5000/)
...
test('draft scan is a multi-finding audit distinct from the scene scan', async () => {
  ...
  assert.match(editor, /reconcileDraftScanResult/)
  assert.match(editor, /canReuseIssueLedger/)
  assert.match(editor, /anchor-issue-ledger:/)
  assert.match(editor, /anchor-story-memory:/)
  assert.match(editor, /\.slice\(0, 5\)/)
...
test('same-user auth refreshes preserve and restore the open project', async () => {
  ...
  assert.match(layout, /Editor 0\.4\.8 Inky Motion/)
})
...
test('release configuration documents every AI model and enforces CI', async () => {
  ...
  assert.match(workflow, /github\.actor == 'emailjustin13-beep'/)
  assert.match(workflow, /\[live-gauntlet\]/)
  assert.match(workflow, /secrets\.ANTHROPIC_API_KEY/)
  assert.match(workflow, /--mode live --repeat 1/)
  ...
  assert.match(readme, /10 permanent Gauntlet screenplays/)
  assert.match(readme, /34 chronological revisions/)
